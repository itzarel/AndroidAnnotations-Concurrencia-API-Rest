...
public class MainActivity extends AppCompatActivity {
    ...
    private int counterAvenger = 0, allAveners = 0;
    ...
    void countAllAvengersBackground() {
        allAveners = receiverAction.totalAvengers();
        Log.i("MainActivity", "Recovered number of avengers: " + allAveners);
    }
    ...
    void nextAvenger() {
        if (hero != null) {
            counterAvenger = (counterAvenger + 2) % allAveners;
    ...
    void backAvenger() {
        if (hero != null) {
            counterAvenger = (counterAvenger - 2) % allAveners;
            if (counterAvenger < 0) {
                counterAvenger = allAveners + counterAvenger;
            }
            recoverAvengerBackground(counterAvenger);
        }
    }
    ...
    void addAvenger() {
        addAvengerBackground();
    }

    @Click(R.id.fab_menu_update)
    void updateAvenger() {
        updateAvengerBackground();
    }
    ...
    @Background(serial = "mainProcessAvengers")
    void addAvengerBackground() {
        counterAvenger = 0;

        long newId = receiverAction.getLastId() + 1;
        Avenger newAvengerEN = createFakeAvenger(newId, "Encoded name is <", "Actor code <", "Encrpted information <");
    ...
    @Background(serial = "mainProcessAvengers")
    void updateAvengerBackground() {
        counterAvenger = 0;

        Avenger newAvenger = updateFakeAvenger();
    ...
    private Avenger createFakeAvenger(long newId, String name, String actor, String desc) {
        ...
        return new Avenger(newId,
                Constants.EN,
                name + rand.nextInt(500000) + CLOSE_TAG,
                actor + rand.nextInt(500000) + " "  + rand.nextInt(500000) + rand.nextInt(500000) + CLOSE_TAG,
                desc + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +
                        rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +
                        rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +
                        rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) +  + rand.nextInt(500000) + CLOSE_TAG,
    ...
    private Avenger updateFakeAvenger() {
        final  String CLOSE_TAG = ">";
        Random rand = new Random();
        String newName;

        if (hero.getHero().contains(CLOSE_TAG)) {
            newName = hero.getHero().replace(CLOSE_TAG, " " + rand.nextInt(500000) + CLOSE_TAG);
        } else {
            newName = "<" + hero.getHero() + " " + rand.nextInt(500000) + CLOSE_TAG;
        }
