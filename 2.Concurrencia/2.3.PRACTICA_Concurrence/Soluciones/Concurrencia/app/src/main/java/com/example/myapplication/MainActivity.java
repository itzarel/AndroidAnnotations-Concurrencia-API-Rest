...
public class MainActivity extends AppCompatActivity {
    ...
    @IntegerRes
    int prime_input_value, fibonacci_size_input_value;

    @StringRes
    String only_natural_numbers, prime_number,
            no_prime_number, fibonacci_succession;
    ...
    void prime() {
        ...
        if (prime_input_value < 0) {
            ...
            if (prime_input_value <= 3) {
                ...
                for (int c = 2; c < prime_input_value; c++) {
    ...
    @UiThread
    void updateTvPrimeUiThread(String result) {
        tv_prime.setText(result);
    }

    @UiThread
    void updateTvFibonacciUiThread(String result) {
        tv_fibonacci.setText(result);
    }
    ...
    private double calculateFibonacci(double fibonacciElement) {
        if (fibonacciElement > 1) {
            return calculateFibonacci(fibonacciElement-1) +
                    calculateFibonacci(fibonacciElement-2);
